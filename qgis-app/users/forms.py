...
class QgisUserForm(MapModelForm):
    ...
    email = forms.EmailField(
        ...
        error_messages={"required": "Please enter the email address you wish to use"},
    )
    ...
    geometry = forms.CharField(
        label="User location:",
        required=True,
        widget=EditableMap(options={"layers": ["osm.mapnik"]}),
...
class EmailForm(forms.Form):
    email = forms.EmailField(
        ...
        required=True,
        help_text="",
        error_messages={"required": "A valid email is required"},
    )
